...
unsigned int process_resource(unsigned int client_id, unsigned int resource_id) {
    ...
    if(sem_wait(&semaphores[resource_id]) != 0) printf("ERRORE sem_wait");
    ...
    if(sem_post(&semaphores[resource_id]) != 0) printf("ERRORE sem_post");
...
void* connection_handler(void* arg) {
    ...
    if(close(socket_desc) != 0) printf("ERRORE colse");
    free(args);
...
int main(int argc, char* argv[]) {
    ...
    for(i = 0; i<N; i++){
        ...
        if(ret != 0) printf("ERRORE sem_int");
    }
    ...
    while (1) {
		 ...
		 **/
         ...
         if(ret != 0) printf("ERRORE pthread_detach");
