...
unsigned int process_resource(unsigned int client_id, unsigned int resource_id) {
    ...
    if(sem_wait(&sem[resource_id]) == -1) handle_error("Errore sem_wait");
    ...
    if(sem_post(&sem[resource_id]) == -1) handle_error("Errore sem_wait");
...
void* connection_handler(void* arg) {
    ...
    while (1) {
        ...
        bytes_read = 0;
        do {
            ret = recv(client_id, buf+bytes_read, 1, 0);
        ...
        while(bytes_sent < msg_len){
            ret = send(client_id, buf+bytes_sent, msg_len-bytes_sent, 0);
            if(ret == -1 && errno == EINTR) continue;
    ...
    if(close(socket_desc) == -1) handle_error("Errore close");
    free(arg);
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        if(ret != 0) handle_error_en(ret, "Errore pthread_detach");
