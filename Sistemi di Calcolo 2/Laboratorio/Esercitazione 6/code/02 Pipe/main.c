...
int write_to_pipe(int fd, const void *data, size_t data_len) {
    ...
    while(written_bytes < data_len){
        ...
        if(ret == -1)handle_error("[write_to_pipe] Errore write");
        written_bytes += ret;
    }
...
int read_from_pipe(int fd, void *data, size_t data_len) {
    ...
    while(read_bytes < data_len){
        ...
        if(ret == -1 && errno == EINTR) continue;
        if(ret == -1)handle_error("[read_from_pipe] Errore read");
        if(ret ==  0)handle_error("[read_from_pipe] Errore read");
        read_bytes += ret;
    }
...
void writer(int writer_id, sem_t* write_mutex) {
    ...
    ret = close(pipefd[0]);
    if(ret == -1) handle_error("[Writer] Errore close");
...
int main(int argc, char* argv[]) {
     ...
     **/
    ret = pipe(pipefd);
    if(ret == -1) handle_error("[Main] Errore pipe");
