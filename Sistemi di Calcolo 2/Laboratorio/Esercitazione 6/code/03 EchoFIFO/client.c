...
int main(int argc, char* argv[]) {
     ...
     **/
     ...
     if(echo_fifo == -1) handle_error("[main client] errore open echo fifo");
     client_fifo = open(CLNT_FIFO_NAME, O_WRONLY);
     if(client_fifo == -1) handle_error("[main client] errore open client fifo");
    ...
    while (1) {
        ...
        if (bytes_left - 1 == quit_command_len && !memcmp(buf, quit_command, quit_command_len)) break;
