...
int main(int argc, char* argv[]) {
    ...
    if(socket_desc == -1) handle_error("[Client] Errore creazione socket");
     ...
     * - - it requires as second field struct sockaddr* addr, but our address is
     *     a struct sockaddr_in, hence we must cast it (struct sockaddr*) &server_addr
     */
    ...
    if(ret == -1) handle_error("[Client] Errore connect");
    ...
    do {
        ...
        if(ret == -1) handle_error("[Client] Errore recv");
        if(ret == 0) break;
        recv_bytes += ret;
...
//        buf[--msg_len] = '\0'; // remove '\n' from the end of the message
        ...
        while(bytes_sent < msg_len){
            ...
            if(ret == -1 && errno == EINTR) continue;
            if(ret == -1) handle_error("[Client] Errore send");
            bytes_sent += ret;
        }
    ...
    if(ret == -1) handle_error("[Client] Errore close");
