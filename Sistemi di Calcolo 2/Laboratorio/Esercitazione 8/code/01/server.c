...
void* connection_handler(int socket_desc) {
    ...
    while (1) {
        ...
        recv_bytes = 0;
        do {
            ret = recv(socket_desc, buf+recv_bytes, 1, 0);      //pattern che legge un byte per volta, perchè non conosce la size
            if(ret == -1 && errno == EINTR) continue;
        ...
        if(recv_bytes == quit_command_len){
            ret = memcmp(buf, quit_command, quit_command_len);
            if(ret == 0) break;
        }
    ...
    if(ret == -1) handle_error("[Server] Errore close");
...
int main(int argc, char* argv[]) {
    ...
    if(socket_desc == -1) handle_error("[Server] Errore creazione socket");
     ...
     * - - it requires as second field struct sockaddr* addr, but our address is a
     *     struct sockaddr_in, hence we must cast it (struct sockaddr*) &server_addr
     */
    ...
    if(ret == -1) handle_error("[Server] Errore bind");
    ...
    */
    ...
    if(ret == -1) handle_error("[Server] Errore listen");
    ...
    while (1) {
        ...
        if(client_desc == -1) handle_error("[Server] Errore accept");
