...
void initSemaphores() {
    ...
    */
    ...
    sem_filled = NULL;                                                  //ESERCIZIO inzio
...
void closeSemaphores() {
     ...
     * possible errors using the predefined handle_error() macro */
     ...
     if(sem_close(sem_filled)){
        handle_error("Producer - Cannot close sem_filled.");
     }
     if(sem_close(sem_empty)){
        handle_error("Producer - Cannot close sem_empty.");
     }
     if(sem_close(sem_cs)){
        handle_error("Producer - Cannot close sem_cs.");
     }                                                             //ESERCIZIO fine
...
void produce(int id, int numOps) {
    ...
    while (numOps > 0) {
        ...
        if(sem_wait(sem_empty)){                                        //ESERCZIO inizio
            handle_error("Producer - Cannot do sem_wait(sem_empty).");
        }
        if(sem_wait(sem_cs)){
            handle_error("Producer - Cannot do sem_wait(sem_cs).");
        }
        ...
        if(sem_post(sem_filled)){
            handle_error("Producer - Cannot do sem_post(sem_filled).");
        }                                                               //ESERCIZIO fine
