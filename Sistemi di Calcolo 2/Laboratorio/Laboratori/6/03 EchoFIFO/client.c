...
int main(int argc, char* argv[]) {
    ...
    if(echo_fifo == -1) handle_error("open fifo error");
    client_fifo = open(CLNT_FIFO_NAME, O_WRONLY);
    if(client_fifo == -1) handle_error("open fifo error");
    ...
    bytes_read = 0;
    do{
        ret = read(echo_fifo, buf+bytes_read, 1);
    ...
    while (1) {
        ...
        while(bytes_read < bytes_sent){
            ret = read(echo_fifo, buf+bytes_read, bytes_sent-bytes_read);
            if(ret == -1 && errno == EINTR) continue;
            if(ret == -1) handle_error("read error");
            if(ret == 0) handle_error("unexpeted close");
            bytes_read += ret;
