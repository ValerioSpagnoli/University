...
void connection_handler(int socket_desc, struct sockaddr_in* client_addr) {
    int ret, recv_bytes = 0, bytes_sent = 0;
...
void mthreadServer(int server_desc) {
    ...
    while (1) {
        ...
        ret = pthread_create(&thread, NULL, thread_connection_handler, args);
        if(ret == -1) handle_error("pthread_create error");
        ret = pthread_detach(thread);
        if(ret == -1) handle_error("pthread detach error");
