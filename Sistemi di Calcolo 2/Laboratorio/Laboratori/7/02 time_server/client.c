...
int main(int argc, char* argv[]) {
    ...
    int sent_bytes = 0;
    while(sent_bytes < command_len){
        ret = send(socket_desc, command+sent_bytes, command_len-sent_bytes, 0);
    ...
    while(recv_bytes < 1){
        ...
        if(ret == -1 && errno == EINTR) continue;
        if(ret == -1) handle_error("recv error1");
        if(ret == 0) handle_error("recv error2");
        recv_bytes += ret;
    }
